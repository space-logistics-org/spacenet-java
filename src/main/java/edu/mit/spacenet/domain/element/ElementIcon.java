...
/**
 * The element icon enumeration serves as a library of built-in icons for elements.
 * 
 * @author Paul Grogan
 */
public enum ElementIcon {
  ...
  public static ElementIcon getInstance(ImageIcon icon) {
    for (ElementIcon elementIcon : values()) {
      if (elementIcon.getIcon().equals(icon))
        return elementIcon;
    }
    return null;
  }
  ...
  public static ElementIcon getInstance(String string) {
    if (string != null) {
      for (ElementIcon icon : values()) {
        if (icon.getName().toLowerCase().equals(string.toLowerCase()))
          return icon;
      }
    }
    return null;
