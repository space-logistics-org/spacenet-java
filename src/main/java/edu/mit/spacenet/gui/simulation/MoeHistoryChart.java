...
/**
 * Chart for visualizing the evolution of the various measures of effectiveness over the course of
 * the simulation.
 * 
 * @author Paul Grogan
 */
public class MoeHistoryChart extends JSplitPane {
    ...
    switch ((MoeType) moeCombo.getSelectedItem()) {
      case CREW_SURFACE_DAYS:
        for (MoeCrewSurfaceDays moe : tab.getSimulator().getCrewSurfaceDaysHistory()) {
          locationsModel.addObject(moe.getLocation(),
              !deselectedLocations.contains(moe.getLocation()));
        }
      ...
      case LAUNCH_MASS:
        ...
        format = new DecimalFormat("0.0");
        totalValueLabel
            .setText(format.format(tab.getSimulator().getTotalLaunchMass() / 1000d) + " mT");
        break;
      case EXPLORATION_MASS_DELIVERED:
        for (MoeExplorationMassDelivered moe : tab.getSimulator()
            .getExplorationMassDeliveredHistory()) {
          locationsModel.addObject(moe.getLocation(),
              !deselectedLocations.contains(moe.getLocation()));
        }
        format = new DecimalFormat("0");
        totalValueLabel
            .setText(format.format(tab.getSimulator().getTotalExplorationMassDelivered()) + " kg");
        break;
      case UP_MASS_CAPACITY_UTILIZATION:
        for (MoeMassCapacityUtilization moe : tab.getSimulator()
            .getUpMassCapacityUtilizationHistory()) {
          locationsModel.addObject(moe.getLocation(),
              !deselectedLocations.contains(moe.getLocation()));
        }
        format = new DecimalFormat("0.000");
        totalValueLabel
            .setText(format.format(tab.getSimulator().getTotalUpMassCapacityUtilization()));
        break;
      case RETURN_MASS_CAPACITY_UTILIZATION:
        for (MoeMassCapacityUtilization moe : tab.getSimulator()
            .getDownMassCapacityUtilizationHistory()) {
          locationsModel.addObject(moe.getLocation(),
              !deselectedLocations.contains(moe.getLocation()));
        }
        format = new DecimalFormat("0.000");
        totalValueLabel
            .setText(format.format(tab.getSimulator().getTotalDownMassCapacityUtilization()));
        break;
      ...
      case RELATIVE_EXPLORATION_CAPABILITY:
        ...
        totalValueLabel
            .setText(format.format(tab.getSimulator().getTotalRelativeExplorationCapability()));
        break;
    }
    updateChart();
  ...
  private void updateChart() {
    ...
    JFreeChart chart =
        ChartFactory.createTimeSeriesChart(null, "Date", "Value", dataset, true, true, false);
    ...
    if (((MoeType) moeCombo.getSelectedItem()).equals(MoeType.CREW_SURFACE_DAYS)) {
      ...
      s.addOrUpdate(
          new Hour(
              DateFunctions.getDate(getScenario().getStartDate(), tab.getSimulator().getTime())),
          amount);
      dataset.addSeries(s);
      ...
      for (MoeCrewTime moe : tab.getSimulator().getCrewTimeHistory()) {
        Date date = DateFunctions.getDate(getScenario().getStartDate(), moe.getTime());
        if (locationsModel.getSelectedObjects().contains(moe.getLocation())) {
          if (!linearizeDataCheck.isSelected() && amount1 > 0)
            s1.addOrUpdate(new Hour(date).previous(), amount1);
          if (!linearizeDataCheck.isSelected() && amount2 > 0)
            s2.addOrUpdate(new Hour(date).previous(), amount2);
          if (!linearizeDataCheck.isSelected() && amount3 > 0)
            s3.addOrUpdate(new Hour(date).previous(), amount3);
          if (!linearizeDataCheck.isSelected() && amount4 > 0)
            s4.addOrUpdate(new Hour(date).previous(), amount4);
    ...
    } else if (((MoeType) moeCombo.getSelectedItem())
        .equals(MoeType.UP_MASS_CAPACITY_UTILIZATION)) {
      ...
      for (MoeMassCapacityUtilization moe : tab.getSimulator()
        ...
        Date date = DateFunctions.getDate(getScenario().getStartDate(), moe.getTime());
        if (locationsModel.getSelectedObjects().contains(moe.getLocation())) {
          if (!linearizeDataCheck.isSelected() && capacity > 0)
            s1.addOrUpdate(new Hour(date).previous(), capacity);
          if (!linearizeDataCheck.isSelected() && amount > 0)
            s2.addOrUpdate(new Hour(date).previous(), amount);
      ...
      valueLabel.setText(capacity == 0 ? "n/a" : format.format(amount / capacity));
        ...
        .equals(MoeType.RETURN_MASS_CAPACITY_UTILIZATION)) {
      TimeSeries s1 = new TimeSeries("Capacity", Hour.class);
      TimeSeries s2 = new TimeSeries("Utilization", Hour.class);
      ...
      valueLabel.setText(format.format(amount));
        ...
        .equals(MoeType.RELATIVE_EXPLORATION_CAPABILITY)) {
      TimeSeries s = new TimeSeries("Total", Hour.class);
      ...
      for (MoeExplorationCapability moe : tab.getSimulator().getExplorationCapabilityHistory()) {
        ...
        if (locationsModel.getSelectedObjects().contains(moe.getLocation())) {
          ...
          if (!linearizeDataCheck.isSelected() && value > 0)
            s.addOrUpdate(new Hour(date).previous(), value);
          value = (amount / FullSimulator.APOLLO_17_EXPLORATION_CAPABILITY)
              / (mass / FullSimulator.APOLLO_17_LAUNCH_MASS);
