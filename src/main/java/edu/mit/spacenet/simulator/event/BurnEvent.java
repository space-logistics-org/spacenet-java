...
/**
 * Event that represents a propulsive maneuver that may be composed of one or more burns or stages
...
public class BurnEvent extends AbstractEvent {
  ...
  /**
   * The default constructor initializes the items and burn stage sequence structures.
   */
  public BurnEvent() {
    super();
  ...
  public void execute(I_Simulator simulator) throws SimError {
    ...
    for (BurnStageItem i : burnStageSequence) {
      if (i.getBurnStage().equals(BurnStageItem.BURN)) {
        ...
        if (burn.getBurnType() == BurnType.OMS) {
          System.out.printf("%.3f: %s %.2f %s", getTime(),
              "Starting " + v.getName() + " Burn (OMS) for target", deltaVReq, "m/s delta-V\n");
          if (Formulae.getRequiredFuelMass(stackMass, deltaVReq, v.getOmsIsp()) > v.getOmsFuelTank()
            ...
            /*
             * DemandSet demands = new DemandSet(); Demand demand = new Demand();
             * demand.setResource(v.getOmsFuelTank().getResource());
             * demand.setAmount(v.getOmsFuelTank().getAmount()); demands.add(demand); DemandEvent d
             * = new DemandEvent(); d.setTime(getTime()); d.setPriority(pcounter++);
             * d.setParent(this); d.setLocation(getLocation()); d.setElement(v.getOmsFuelTank());
             * //d.setElement(v); d.setDemands(demands); //simulator.schedule(d);
             * d.execute(simulator);
             */
            System.out.printf("%.3f: %s %.2f %s %.2f %s\n", getTime(),
                v.getName() + " Burn (OMS) achieved", deltaVAchieved, "m/s delta-V,", deltaVReq,
                "m/s delta-V remaining");
          } else {
            v.getOmsFuelTank()
                .remove(Formulae.getRequiredFuelMass(stackMass, deltaVReq, v.getOmsIsp()));
            /*
             * DemandSet demands = new DemandSet(); Demand demand = new Demand();
             * demand.setResource(v.getOmsFuelTank().getResource());
             * demand.setAmount(Formulae.getRequiredFuelMass(stackMass, deltaVReq, v.getOmsIsp()));
             * demands.add(demand); DemandEvent d = new DemandEvent(); d.setTime(getTime());
             * d.setPriority(pcounter++); d.setParent(this); d.setLocation(getLocation());
             * d.setElement(v.getOmsFuelTank()); //d.setElement(v); d.setDemands(demands);
             * //simulator.schedule(d); d.execute(simulator);
             */
            System.out.printf("%.3f: %s %.2f %s, %.2f %s\n", getTime(),
                v.getName() + " Burn (OMS) achieved", deltaVReq, "m/s delta-V",
                v.getOmsFuelTank().getCargoMass(), "kg fuel remaining");
            deltaVReq = 0;
          }
        } else if (burn.getBurnType() == BurnType.RCS) {
          System.out.printf("%.3f: %s %.2f %s", getTime(),
              "Starting " + v.getName() + " Burn (RCS) for target", deltaVReq, "m/s delta-V\n");
          if (Formulae.getRequiredFuelMass(stackMass, deltaVReq, v.getRcsIsp()) > v.getRcsFuelTank()
             ...
             * demand.setAmount(v.getRcsFuelTank().getAmount()); demands.add(demand); DemandEvent d
             * = new DemandEvent(); d.setTime(getTime()); d.setPriority(pcounter++);
             * d.setParent(this); d.setLocation(getLocation()); d.setElement(v.getRcsFuelTank());
             * //d.setElement(v); d.setDemands(demands); //simulator.schedule(d);
             * d.execute(simulator);
             */
            System.out.printf("%.3f: %s %.2f %s %.2f %s\n", getTime(),
             ...
             * demands.add(demand); DemandEvent d = new DemandEvent(); d.setTime(getTime());
             * d.setPriority(pcounter++); d.setParent(this); d.setLocation(getLocation());
             * d.setElement(v.getRcsFuelTank()); //d.setElement(v); d.setDemands(demands);
             * //simulator.schedule(d); d.execute(simulator);
             */
            System.out.printf("%.3f: %s %.2f %s, %.2f %s\n", getTime(),
                v.getName() + " Burn (RCS) achieved", deltaVReq, "m/s delta-V",
                v.getRcsFuelTank().getCargoMass(), "kg fuel remaining");
            deltaVReq = 0;
          }
        }
    ...
    if (deltaVReq > 0) {
      ...
      throw new SimSpatialError(simulator.getTime(), this,
          "Insufficient delta-v achieved, - remaining delta-v: " + format.format(deltaVReq)
              + " m/s.");
    }
  }
  ...
  public void print(int tabOrder) {
    ...
    for (BurnStageItem i : burnStageSequence) {
      if (i.getBurnStage().equals(BurnStageItem.BURN))
        s += i.getElement().getName() + " [B], ";
      else if (i.getBurnStage().equals(BurnStageItem.STAGE))
        s += i.getElement().getName() + " [S], ";
    }
    s = s.substring(0, s.length() - 2);
    super.print(tabOrder);
    System.out.printf("%s% .1f %s\n", burn.getBurnType() + " Burn for", burn.getDeltaV(),
        "delta-V: " + s);
  }
