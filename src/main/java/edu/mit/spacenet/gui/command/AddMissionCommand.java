...
/**
 * The command to add a mission to a scenario, pre-filling data as available and triggering a
 * secondary command to edit the newly-created mission.
 * 
 * @author Paul Grogan
 */
public class AddMissionCommand implements I_Command {
  ...
  public void execute() {
    if (missionsTab.getScenarioPanel().getScenario().getMissionList().size() > 0) {
      Mission prevMission = missionsTab.getScenarioPanel().getScenario().getMissionList()
          .get(missionsTab.getScenarioPanel().getScenario().getMissionList().size() - 1);
      mission.setStartDate(
          DateFunctions.getDate(prevMission.getStartDate(), prevMission.getDuration()));
      ...
      if (missionsTab.getScenarioPanel().getScenario().getNetwork().getNodes().size() > 0) {
        mission.setOrigin(
            missionsTab.getScenarioPanel().getScenario().getNetwork().getNodes().first());
