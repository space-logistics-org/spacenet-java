...
public abstract class Edge extends Location {
   ...
   * 
   * @see
   * edu.mit.spacenet.domain.network.Location#compareTo(edu.mit.spacenet.domain.network.Location)
   */
  public int compareTo(Location location) {
    if (location instanceof Edge) {
      return super.compareTo(location);
      /*
       * TODO ordering by node origin breaks compatibility Edge edge = (Edge)location;
       * if(getOrigin().equals(edge.getOrigin())) // if both edges have same origin, order by
       * name/id return super.compareTo(location); else // if edges have different origin, order by
       * origin return getOrigin().compareTo(edge.getOrigin());
       */
