...
/**
 * A bat-style chart that displays scenario events. If used from a missions split pane, the control
 * panel is visible, if used from a mission split pane (while editing one mission), it will be
 * hidden.
 * 
 * @author Paul Grogan
 */
public class ProcessBatChart extends JSplitPane {
  ...
  private void buildChart() {
    ...
    startDate.addPropertyChangeListener(new PropertyChangeListener() {
      public void propertyChange(PropertyChangeEvent e) {
        if (e.getPropertyName().equals("date") && isAutoRefresh())
          updateChart();
      }
    });
    ...
    missionsModel.addTableModelListener(new TableModelListener() {
      public void tableChanged(TableModelEvent e) {
        if (e.getType() == TableModelEvent.UPDATE && isAutoRefresh())
          updateChart();
      }
    });
    JTable missionsTable = new JTable(missionsModel);
    missionsTable.getColumnModel().getColumn(0)
    ...
    eventsTable.getColumnModel().getColumn(0)
        .setHeaderRenderer(new VisibilityTableCellHeaderRenderer());
    eventsTable.getColumnModel().getColumn(1).setCellRenderer(new DefaultTableCellRenderer() {
      ...
      public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected,
        ...
        if (value instanceof EventType)
          setIcon(((EventType) value).getIcon());
        return this;
      }
    });
  ...
  /**
   * Updates the view. (Refreshes the chart if auto-refresh is enabled, ensures manual refresh
  ...
  /**
   * Refreshes the chart (time-intensive for large campaigns). Uses a swing worker to run the update
   * process in a separate thread so the GUI does not freeze.
   */
  ...
  private void rebuildChart() {
    ...
    for (Mission mission : missionsModel.getSelectedObjects()) {
      for (I_Event event : mission.getEventList()) {
        if ((startDate.getDate() == null
            || DateFunctions
                .getDate(getScenario().getStartDate(),
                    DateFunctions.getDaysBetween(getScenario().getStartDate(),
                        mission.getStartDate()) + event.getTime())
                .compareTo(startDate.getDate()) >= 0)
            && (endDate.getDate() == null || DateFunctions
                .getDate(getScenario().getStartDate(),
                    DateFunctions.getDaysBetween(getScenario().getStartDate(),
                        mission.getStartDate()) + event.getTime())
                .compareTo(endDate.getDate()) <= 0)
          ...
          if (event instanceof SpaceTransport && ((SpaceTransport) event).getEdge() != null
            ...
            r.setSeriesShape(dataset.getSeriesCount() - 1,
                new Ellipse2D.Double(-2.0, -2.0, 4.0, 4.0));
          } else if (event instanceof FlightTransport && ((FlightTransport) event).getEdge() != null
    ...
    if (waitCheck.isSelected()) {
      ...
      Mission firstMission =
          missionsModel.getSelectedObjects().size() > 0 ? missionsModel.getSelectedObjects().get(0)
              : null;
      double minTime1 = firstMission == null ? Double.MAX_VALUE
          : DateFunctions.getDaysBetween(getScenario().getStartDate(), firstMission.getStartDate());
      Mission lastMission = missionsModel.getSelectedObjects().size() > 0
          ? missionsModel.getSelectedObjects().get(missionsModel.getSelectedObjects().size() - 1)
          : null;
      double maxTime1 = lastMission == null ? 0
          : DateFunctions.getDaysBetween(getScenario().getStartDate(),
              DateFunctions.getDate(lastMission.getStartDate(), lastMission.getDuration()));

      double minTime2 = startDate.getDate() == null ? 0
          : DateFunctions.getDaysBetween(getScenario().getStartDate(), startDate.getDate());
      double maxTime2 = endDate.getDate() == null ? Double.MAX_VALUE
          : DateFunctions.getDaysBetween(getScenario().getStartDate(), endDate.getDate());
      ...
      for (I_Element element : getScenario().getElements()) {
        ...
        int i = -1;
        for (SimState state : missionsSplitPane.getMissionsTab().getSimulator()
            .getLocationHistory()) {
          if (prevNode != null && prevNode.equals(state.getLocation(element))) {
            series.get(i).addOrUpdate(
                new Hour(DateFunctions.getDate(getScenario().getStartDate(),
                    Math.min(Math.max(minTime, state.getTime()), maxTime))),
