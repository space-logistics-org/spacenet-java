...
public class DemandModelDialog extends JDialog {
  ...
  private void buildDialog() {
    ...
    okButton = new JButton("OK",
        new ImageIcon(getClass().getClassLoader().getResource("icons/comment_go.png")));
  ...
  private boolean isDemandModelValid() {
    if (nameText.getText().length() == 0) {
      JOptionPane.showMessageDialog(this, "Please enter a demand model name.", "SpaceNet Warning",
          JOptionPane.WARNING_MESSAGE);
      return false;
    }
    if (!demandModelPanel.isDemandModelValid())
      return false;
    return true;
  }
  ...
  private void saveDemandModel() {
    if (isDemandModelValid()) {
      ...
      if (missionPanel != null) {
        if (!missionPanel.getMissionSplitPane().getMission().getDemandModels()
            .contains(demandModel)) {
