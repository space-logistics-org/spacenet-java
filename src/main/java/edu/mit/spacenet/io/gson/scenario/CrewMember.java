...
public class CrewMember extends Element {
	...
	public edu.mit.spacenet.domain.element.CrewMember toSpaceNet(Context context) {
		...
		e.setAvailableTimeFraction(availableTimeFraction == null ? template.availableTimeFraction : availableTimeFraction);
		return e;
	}
	
	@Override
	public ElementPreview getPreview(Context context) {
		return new ElementPreview(context.getJavaId(id), name, ElementType.CREW_MEMBER, ElementIcon.getInstance(icon));
	}
