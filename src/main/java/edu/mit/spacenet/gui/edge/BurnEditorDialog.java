...
public class BurnEditorDialog extends JDialog {
  ...
  public void buildDialog() {
    ...
    for (BurnType t : BurnType.values())
      burnTypeCombo.addItem(t);
    burnTypeCombo.setToolTipText(
        "Type of burn: orbital maneuvering system (OMS) or reaction control system (RCS)");
    ...
    okButton = new JButton(burn.getTid() < 0 ? "Save" : "Save",
  ...
  public void saveBurn() {
    if (isBurnValid()) {
      try {
        ...
        e.printStackTrace();
        JOptionPane.showMessageDialog(panel.getDialog(), "SpaceNet Errror",
  ...
  public boolean isBurnValid() {
    if (deltaVModel.getNumber().doubleValue() == 0) {
      JOptionPane.showMessageDialog(this, "Please select a non-zero delta-v.", "SpaceNet Warning",
