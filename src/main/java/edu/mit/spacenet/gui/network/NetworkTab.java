...
public class NetworkTab extends JSplitPane {
  ...
  private JCheckBox drawOrbitalLimitsCheck, simpleModeCheck, displaySunCheck, blackBackgroundCheck;
      ...
      public void valueChanged(ListSelectionEvent e) {
        if (nodeTable.getSelectedRow() >= 0) {
          ...
          for (int row : nodeTable.getSelectedRows()) {
            networkPanel.getNetworkLayer().getSelectedNodes()
                .add(((Node) nodeTable.getModel().getValueAt(row, 1)).getTid());
          }
          networkPanel.repaint();
        }
        if (nodeTable.getSelectedRows().length == 1) {
          nodeDetailsPanel
              .setNode((Node) nodeTable.getModel().getValueAt(nodeTable.getSelectedRow(), 1));
        } else {
      ...
      public void valueChanged(ListSelectionEvent e) {
        if (edgeTable.getSelectedRow() >= 0) {
          ...
          for (int row : edgeTable.getSelectedRows()) {
            networkPanel.getNetworkLayer().getSelectedEdges()
                .add(((Edge) edgeTable.getModel().getValueAt(row, 1)).getTid());
          }
          networkPanel.repaint();
        }
        if (edgeTable.getSelectedRows().length == 1) {
          edgeDetailsPanel
              .setEdge((Edge) edgeTable.getModel().getValueAt(edgeTable.getSelectedRow(), 1));
        } else {
  ...
  private void buildDisplayPanel() {
    ...
    c.fill = GridBagConstraints.HORIZONTAL;
    c.weightx = 1;
    final SpinnerNumberModel sizeScalingModel = new SpinnerNumberModel(
        networkPanel.getBodyLayer().getSizeScale(), 0, Double.MAX_VALUE, 0.1);
    ...
    c.gridy++;
    final SpinnerNumberModel sizeBasisModel = new SpinnerNumberModel(
        networkPanel.getBodyLayer().getSizeBasis(), 0, Double.MAX_VALUE, 0.1);
    final JSpinner sizeBasisSpinner = new JSpinner(sizeBasisModel);
    sizeBasisSpinner.setPreferredSize(new Dimension(60, 20));
    sizeBasisSpinner
        .setToolTipText("A parameter that sets the median size (units of Earth diameters).");
    ...
    c.gridy++;
    final SpinnerNumberModel sizeDisparityModel = new SpinnerNumberModel(
        networkPanel.getBodyLayer().getSizeDisparity(), 1.0, Double.MAX_VALUE, 0.1);
    ...
    sizeDisparitySpinner.setToolTipText(
        "A parameter that sets the maximum difference between the largest and smallest sizes.");
    ...
    c.gridy = 0;
    c.gridx += 2;
    final SpinnerNumberModel locationScalingModel = new SpinnerNumberModel(
        networkPanel.getBodyLayer().getLocationScale(), 0, Double.MAX_VALUE, 0.1);
    ...
    locationScalingSpinner
        .setToolTipText("A parameter that controls the rate of distance increase.");
    locationScalingSpinner.addChangeListener(new ChangeListener() {
      public void stateChanged(ChangeEvent e) {
        networkPanel.getBodyLayer()
            .setLocationScale(locationScalingModel.getNumber().doubleValue());
        networkPanel.repaint();
      }
    });
    bodyOptionsPanel.add(locationScalingSpinner, c);
    c.gridy++;
    final SpinnerNumberModel locationBasisModel = new SpinnerNumberModel(
        networkPanel.getBodyLayer().getLocationBasis(), 0, Double.MAX_VALUE, 0.1);
    ...
    c.gridy++;
    final SpinnerNumberModel locationDisparityModel = new SpinnerNumberModel(
        networkPanel.getBodyLayer().getLocationDisparity(), 1.0, Double.MAX_VALUE, 0.1);
    ...
    locationDisparitySpinner.setToolTipText(
        "A parameter that sets the maximum difference between the largest and smallest distances.");
    locationDisparitySpinner.addChangeListener(new ChangeListener() {
      public void stateChanged(ChangeEvent e) {
        networkPanel.getBodyLayer()
            .setLocationDisparity(locationDisparityModel.getNumber().doubleValue());
        networkPanel.repaint();
      }
    });
    ...
    displaySunCheck.setSelected(
        !networkPanel.getBodyLayer().isSimpleMode() && networkPanel.getBodyLayer().isDisplaySun());
    ...
    c.anchor = GridBagConstraints.LINE_START;
    c.weightx = 1;
    final SpinnerNumberModel altitudeScalingModel = new SpinnerNumberModel(
        1 / networkPanel.getNetworkLayer().getAltitudeScale(), 0.5, Double.MAX_VALUE, 0.5);
    ...
    altitudeScalingSpinner.setToolTipText(
        "A parameter that controls the rate of distance increase for orbital altitude.");
    altitudeScalingSpinner.addChangeListener(new ChangeListener() {
      public void stateChanged(ChangeEvent e) {
        networkPanel.getNetworkLayer()
            .setAltitudeScale(1 / altitudeScalingModel.getNumber().doubleValue());
        networkPanel.repaint();
      }
    });
    networkOptionsPanel.add(altitudeScalingSpinner, c);
    c.gridy++;
    final SpinnerNumberModel altitudeBasisModel = new SpinnerNumberModel(
        networkPanel.getNetworkLayer().getAltitudeBasis(), 0, Double.MAX_VALUE, 0.5);
    ...
    altitudeBasisSpinner
        .setToolTipText("A parameter that sets the median altitude (units of kilometers).");
    altitudeBasisSpinner.addChangeListener(new ChangeListener() {
      public void stateChanged(ChangeEvent e) {
        networkPanel.getNetworkLayer()
            .setAltitudeBasis(altitudeBasisModel.getNumber().doubleValue());
        networkPanel.repaint();
      }
    });
    ...
    c.gridy = 0;
    c.gridx += 2;
    final SpinnerNumberModel nodeSizeModel =
        new SpinnerNumberModel(networkPanel.getNetworkLayer().getNodeSize(), 0, 32, 4);
    ...
    c.gridy++;
    final SpinnerNumberModel edgeWidthModel =
        new SpinnerNumberModel(networkPanel.getNetworkLayer().getEdgeWidth(), 1, 5, 1);
  ...
  /**
   * Initializes the network tab by enabling/disabling components and setting initial values.
   */
