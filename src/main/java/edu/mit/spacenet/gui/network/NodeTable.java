    ...
    public int getRowCount() {
      if (scenario != null && scenario.getDataSource() != null) {
        ...
        for (Node node : scenario.getDataSource().getNodeLibrary()) {
          if ((networkTab.getNodeTypeFilter() == null
              || node.getNodeType() == networkTab.getNodeTypeFilter())
              && node.getName().toLowerCase()
                  .contains(networkTab.getNodeSearchText().toLowerCase())) {
            count++;
          }
        }
        return count;
      ...
      if (col == 0) {
        ...
        if (value.equals(Boolean.TRUE)) {
          ...
          if (uses.size() > 0) {
            ...
            warningPanel.add(
                new JLabel("The following use " + node + " and must be changed before removal:"),
                c);
            c.gridy++;
            c.weighty = 1;
            ...
            JOptionPane.showMessageDialog(networkTab.getScenarioPanel(), warningPanel,
