...
public class EvaEventPanel extends AbstractEventPanel {
    ...
    elementTree.addTreeSelectionListener(new TreeSelectionListener() {
      public void valueChanged(TreeSelectionEvent e) {
        getEventDialog().setOkButtonEnabled(
            elementTree.getSelectionCount() > 0 && elementTree.getSelection() instanceof I_Carrier);
        updateView();
      }
    });
  ...
  public void saveEvent() {
    ...
    event.setVehicle(
        (I_Carrier) getEventDialog().getElement(elementTree.getElementSelection().getUid()));

    event.getStateMap().clear();
    for (I_Element simCrew : contentsTable.getModel().getData().keySet()) {
      I_Element crew = (I_Element) getEventDialog().getElement(simCrew.getUid());
      if (crew != null) {
  ...
  public void updateView() {
    ...
    elementTree.setRoot(getEventDialog().getSimNode());
    if (selectedItem != null && getEventDialog().getSimElement(selectedItem.getUid()) != null) {
