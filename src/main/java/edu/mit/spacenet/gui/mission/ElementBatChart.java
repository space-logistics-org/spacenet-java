...
/**
 * A bat-style chart to display the element location history with associated control panel.
 * 
 * @author Paul Grogan
 */
public class ElementBatChart extends JSplitPane {
  ...
  /**
   * Refreshes the chart. Uses a swing worker to run the process in a separate thread so the GUI
   * does not freeze.
   */
  ...
  /**
   * Updates the chart (time-intensive for large campaigns or for many elements.
   */
  private void updateChart() {
    TimeSeriesCollection dataset = new TimeSeriesCollection();

    JFreeChart chart =
        ChartFactory.createTimeSeriesChart(null, "Date", "Node", dataset, true, true, false);
    ...
    for (I_Element element : elementsModel.getSelectedObjects()) {
      ...
      for (SimState state : getSimulator().getLocationHistory()) {
        ...
        if (state.getLocation(element) != null && state.getLocation(element) instanceof Node
            && (startDate.getDate() == null || date.after(startDate.getDate()))
            && (endDate.getDate() == null || date.before(endDate.getDate()))) {
          s.addOrUpdate(new Hour(date),
              delta + node2Number((Node) state.getLocation(element), rangeAxis));
        }
      }
      /*
       * TODO: this section may attempt at removing "unnecessary" points, i.e. the points where the
       * element is at the same nodeboth before and after. for(int i=1; i < s.getItemCount()-1; i++)
       * { if(s.getDataItem(i).getValue().equals(s.getDataItem(i+1).getValue()) &&
       * s.getDataItem(i).getValue().equals(s.getDataItem(i-1).getValue())) {
       * s.delete(s.getTimePeriod(i)); i=1; } }
       */
      if (s.getItemCount() > 0) {
