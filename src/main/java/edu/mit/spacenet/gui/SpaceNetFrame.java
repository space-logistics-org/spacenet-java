...
public class SpaceNetFrame extends JFrame {
  ...
  private SpaceNetFrame() {
        ...
        if (getDialogType() == JFileChooser.SAVE_DIALOG) {
          ...
          if (file != null && file.exists()) {
            int answer = JOptionPane.showOptionDialog(this,
                ...
                "Save Warning", JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE, null, null,
                null);
    ...
    try {
      java.lang.reflect.Method method =
          Class.forName("java.awt.Window").getDeclaredMethod("setIconImages", java.util.List.class);
  ...
  public void paint(Graphics g) {
    ...
    if (scenarioPanel.getScenario() == null)
      setTitle("SpaceNet 2.5r2");
    else
      setTitle("SpaceNet 2.5r2 | " + scenarioPanel.getScenario().getName());
  }
  ...
  public SpaceNetSettingsDialog getSpaceNetSettingsDialog() {
    if (spaceNetSettingsDialog == null)
      spaceNetSettingsDialog = new SpaceNetSettingsDialog();
  ...
  public void openScenario(String filePath) {
    try {
      ...
      JOptionPane.showMessageDialog(this, "The scenario file path (" + filePath + ") is invalid.",
          "SpaceNet Error", JOptionPane.ERROR_MESSAGE);
    } catch (IOException e) {
      JOptionPane.showMessageDialog(this, "The open process failed due to an I/O exception.",
          "SpaceNet Error", JOptionPane.ERROR_MESSAGE);
    }
  }
  ...
  public void saveScenario() {
    try {
      ...
      JOptionPane.showMessageDialog(this, "The save process failed due to an I/O exception.",
          "SpaceNet Error", JOptionPane.ERROR_MESSAGE);
    }
  }

  /**
   * Sets the scenario (and updates the id generator last value) and updates the scenario panel, or
   * closes the scenario if null.
   * 
  ...
  private void setScenario(Scenario scenario) {
    if (scenario == null) {
      ...
      for (I_Element element : scenario.getElements()) {
        if (element.getUid() > lastUid)
          lastUid = element.getUid();
      }
      for (I_Element container : scenario.getManifest().getContainers()) {
        if (container.getUid() > lastUid)
          lastUid = container.getUid();
      }
  ...
  public void exitSpaceNet() {
    if (getScenarioPanel().getScenario() != null) {
      int answer = JOptionPane.showOptionDialog(this,
          "Save '" + scenarioPanel.getScenario().getName() + "' before exiting?", "Close Warning",
