...
public class TimeDependentEdge extends Edge {
  ...
  public List<SpaceEdge> getEdges(Date departureDate, double originPeriapsis, double originApoapsis,
      double destinationPeriapsis, double destinationApoapsis, double departureC3Limit,
      double arrivalC3Limit, boolean usesAerocapture) throws IOException {
    ...
    double departureDateHMD =
        departureDateHour + departureDateMinute / 60 + departureDateSecond / (60 * 60);
    double departureDateJulian = 367 * departureDateYear
        - Math.floor(7 * (departureDateYear + Math.floor((departureDateMonth + 9) / 12)) / 4)
        + Math.floor(275 * departureDateMonth / 9) + departureDateDate + departureDateHMD / 24
        + 1721013.5;
    ...
    int departureDateRowNumber =
        (int) Math.ceil((departureDateJulian - originDateJulian) / xStepSize);
    ...
    for (int j = 0; j <= yCellNum - 1; j++) {
      ...
      if (C3d <= departureC3Limit && C3a <= arrivalC3Limit) {
        ...
        double departureDeltaV = (Math.sqrt(C3d + 2 * muOriginBody / radiusOriginPeriapsis)
            - Math.sqrt(muOriginBody / radiusOriginPeriapsis) * Math
                .sqrt(2 * radiusOriginApoapsis / (radiusOriginPeriapsis + radiusOriginApoapsis)))
            * 1000;
        double arrivalDeltaV = (Math.sqrt(C3a + 2 * muDestinationBody / radiusDestinationPeriapsis)
            - Math.sqrt(muDestinationBody / radiusDestinationPeriapsis)
                * Math.sqrt(2 * radiusDestinationApoapsis
                    / (radiusDestinationPeriapsis + radiusDestinationApoapsis)))
            * 1000;
