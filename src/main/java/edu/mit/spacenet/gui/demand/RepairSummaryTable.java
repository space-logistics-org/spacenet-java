...
public class RepairSummaryTable extends JTable {
  ...
  /**
   * Updates the data within the currently selected row (assumes that only the current mission can
   * be edited).
   */
  ...
  private class RepairSummaryModel extends AbstractTableModel {
    ...
    public Object getValueAt(int row, int col) {
      if (col == 0)
        return row;
      if (col == 1)
        return getScenario().getMissionList().get(row);
      if (col == 2)
        return getDeltaTime(row);
      else
        return getDeltaMass(row);
    }
    ...
    private double getDeltaTime(int row) {
      ...
      if (getScenario().getRepairedItems().get(getScenario().getMissionList().get(row)) != null) {
        for (RepairItem item : getScenario().getRepairedItems()
            .get(getScenario().getMissionList().get(row))) {
