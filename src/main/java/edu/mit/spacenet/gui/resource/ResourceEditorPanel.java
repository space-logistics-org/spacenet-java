...
public class ResourceEditorPanel extends JPanel {
  ...
  private void buildPanel() {
    ...
    for (ClassOfSupply t : ClassOfSupply.values())
      classOfSupplyCombo.addItem(t);
    ...
    packingFactorSpinner.setToolTipText(
        "Fraction of additional COS 5 required to pack resource in a container [unitless]");
  ...
  public boolean isResourceValid() {
    if (nameText.getText().length() == 0) {
      JOptionPane.showMessageDialog(dialog, "Please enter a resource name.", "SpaceNet Warning",
