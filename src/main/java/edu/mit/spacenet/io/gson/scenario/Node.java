...
public abstract class Node extends Location {
  ...
  public static Node createFrom(edu.mit.spacenet.domain.network.node.Node node, Context context) {
    if (node.getNodeType() == NodeType.SURFACE) {
      return SurfaceNode.createFrom((edu.mit.spacenet.domain.network.node.SurfaceNode) node,
          context);
    } else if (node.getNodeType() == NodeType.ORBITAL) {
      return OrbitalNode.createFrom((edu.mit.spacenet.domain.network.node.OrbitalNode) node,
          context);
    } else if (node.getNodeType() == NodeType.LAGRANGE) {
      return LagrangeNode.createFrom((edu.mit.spacenet.domain.network.node.LagrangeNode) node,
          context);
    } else {
  ...
  public static List<Node> createFrom(
      Collection<? extends edu.mit.spacenet.domain.network.node.Node> nodes, Context context) {
  ...
  public static List<edu.mit.spacenet.domain.network.node.Node> toSpaceNet(Collection<Node> nodes,
      Context context) {
    List<edu.mit.spacenet.domain.network.node.Node> ns =
        new ArrayList<edu.mit.spacenet.domain.network.node.Node>();
    if (nodes != null) {
      for (Node n : nodes) {
