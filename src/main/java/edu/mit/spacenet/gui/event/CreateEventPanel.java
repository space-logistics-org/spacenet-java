...
public class CreateEventPanel extends AbstractEventPanel {
  ...
  private JPanel buildLeftPanel() {
    ...
    editElementButton = new JButton("Edit",
        new ImageIcon(getClass().getClassLoader().getResource("icons/brick_edit.png")));
    ...
    removeElementButton.addActionListener(new ActionListener() {
      public void actionPerformed(ActionEvent e) {
        ...
        for (TreePath p : elementTree.getSelectionPaths()) {
          elements
              .add((I_Element) ((DefaultMutableTreeNode) p.getLastPathComponent()).getUserObject());
        }
  ...
  private JPanel buildRightPanel() {
    ...
    elementLibraryList.addMouseListener(new MouseAdapter() {
      public void mouseClicked(MouseEvent e) {
        if (e.getClickCount() == 2 && elementLibraryList.getSelectedIndex() >= 0) {
          try {
            addElement(SpaceNetFrame.getInstance().getScenarioPanel().getScenario().getDataSource()
                .loadElement(elementLibraryList.getSelectedValue().ID));
          } catch (Exception ex) {
            ex.printStackTrace();
            JOptionPane.showMessageDialog(null,
                "An error occurred while accessing the data source to load the element",
                "Error Loading Element", JOptionPane.ERROR_MESSAGE);
          }
        }
      }
    });
    ...
    addElementButton = new JButton("Add",
        new ImageIcon(getClass().getClassLoader().getResource("icons/brick_add.png")));
  ...
  private void addElement(I_Element element) {
    element.setName(getEventDialog().getMissionPanel().getMissionSplitPane().getMission().getName()
        .substring(0, Math.min(getEventDialog().getMissionPanel().getMissionSplitPane().getMission()
            .getName().length(), 5))
        + " | " + element.getName());
    int numElements = 0;
    ...
    if (numElements == 1) {
      for (I_Element existing : elementTree.getModel().getElements()) {
        if (element.getTid() == existing.getTid() && !existing.getName()
            .subSequence(existing.getName().length() - 2, existing.getName().length())
            .equals(" A")) {
    ...
    if (numElements > 0)
      element.setName(element.getName() + " " + getLetter(++numElements));
    elementTree.getModel().addElement(element);
    updateCapacities();
  }
  ...
  private void updateElementLibrary() {
    ...
    for (ElementPreview prev : SpaceNetFrame.getInstance().getScenarioPanel().getScenario()
        .getDataSource().getElementPreviewLibrary()) {
      if ((searchText.getText().equals(searchText.getDefaultText())
          || prev.NAME.toLowerCase().contains(searchText.getText().toLowerCase()))
          && (elementTypeCombo.getSelectedItem().equals("All") || prev.TYPE == (ElementType
              .getInstance((String) elementTypeCombo.getSelectedItem())))) {
