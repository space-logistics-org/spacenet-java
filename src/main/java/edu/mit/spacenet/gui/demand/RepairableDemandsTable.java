...
public class RepairableDemandsTable extends JTable {
    ...
    getColumnModel().getColumn(2).setCellRenderer(new DefaultTableCellRenderer() {
      ...
      public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected,
        ...
        setText(format.format(value) + " "
            + ((RepairableDemandsModel) getModel()).getDemand(row).getResource().getUnits());
        return this;
      }
    });
  ...
  private class RepairableDemandsModel extends AbstractTableModel {
    ...
    public int getRowCount() {
      if (getScenario() == null || getSimulator() == null)
        return 0;
      else if (getSimulator().getUnsortedRepairItems().get(getMissionIndex()) == null)
        return 0;
      else
        return getSimulator().getUnsortedRepairItems().get(getMissionIndex()).size();
    }
    ...
    public Object getValueAt(int row, int column) {
      if (column == 0)
        ...
        return getDemand(row).getMass();
      else if (column == 4)
        return getRepairItem(row).getMassToRepair();
      else if (column == 5)
        return getRepairItem(row).getMeanRepairTime();
      else if (column == 6) {
        HashSet<RepairItem> repairedItems = new HashSet<RepairItem>();
        if (getScenario().getRepairedItems()
            .get(getScenario().getMissionList().get(getMissionIndex())) != null) {
          for (RepairItem item : getScenario().getRepairedItems()
              .get(getScenario().getMissionList().get(getMissionIndex()))) {
            repairedItems.add(new RepairItem(
                new Demand(item.getDemand().getResource(), item.getDemand().getAmount()),
                item.getElement()));
          }
        }
        for (RepairItem simItem : getSimulator().getUnsortedRepairItems().get(getMissionIndex())) {
          ...
          for (RepairItem item : repairedItems) {
            if (simItem.getDemand().getResource().equals(item.getDemand().getResource())
              ...
              double amountNewlyRepaired = Math.min(
                  simItem.getDemand().getAmount() - amountRepaired, item.getDemand().getAmount());
              item.getDemand().setAmount(item.getDemand().getAmount() - amountNewlyRepaired);
              amountRepaired = amountRepaired + amountNewlyRepaired;
            }
          }
          if (getRepairItem(row).equals(simItem))
    ...
    public void setValueAt(Object value, int row, int column) {
      if (column == 6) {
        ...
        if (newValue > oldValue) {
          if (getScenario().getRepairedItems()
              .get(getScenario().getMissionList().get(getMissionIndex())) == null) {
            getScenario().getRepairedItems().put(
                getScenario().getMissionList().get(getMissionIndex()), new HashSet<RepairItem>());
          }
          ...
          if (!isFound) {
            getScenario().getRepairedItems()
                .get(getScenario().getMissionList().get(getMissionIndex()))
                .add(new RepairItem(new Demand(getDemand(row).getResource(), newValue - oldValue),
                    getElement(row)));
          }
