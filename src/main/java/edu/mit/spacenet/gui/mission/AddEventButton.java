...
/**
 * A custom drop down button that contains the menu items for adding all of the different kinds of
 * events to a mission.
 */
public class AddEventButton extends DropDownButton {
  ...
  public AddEventButton(MissionPanel missionPanel) {
    super("Add Event",
        new ImageIcon(missionPanel.getClass().getClassLoader().getResource("icons/cog_add.png")));
  ...
  private void addMenuItems() {
    JMenuItem addCreateEvent =
        new JMenuItem(EventType.CREATE.getName(), EventType.CREATE.getIcon());
    addCreateEvent
        .setToolTipText("Instantiate elements at a node or nested within a carrier element");
    ...
    JMenuItem addTransferEvent =
        new JMenuItem(EventType.TRANSFER.getName(), EventType.TRANSFER.getIcon());
    ...
    JMenuItem addRemoveEvent =
        new JMenuItem(EventType.REMOVE.getName(), EventType.REMOVE.getIcon());
    ...
    JMenuItem addReconfigureEvent =
        new JMenuItem(EventType.RECONFIGURE.getName(), EventType.RECONFIGURE.getIcon());
    ...
    JMenuItem addReconfigureGroupEvent =
        new JMenuItem(EventType.RECONFIGURE_GROUP.getName(), EventType.RECONFIGURE_GROUP.getIcon());
    ...
    JMenuItem addDemandEvent =
        new JMenuItem(EventType.DEMAND.getName(), EventType.DEMAND.getIcon());
    ...
    JMenuItem addSpaceTransport =
        new JMenuItem(EventType.SPACE_TRANSPORT.getName(), EventType.SPACE_TRANSPORT.getIcon());
    addSpaceTransport.setToolTipText(
        "Transports a stack from node to node along a trajectory of impulsive burns");
    ...
    JMenuItem addSurfaceTransport =
        new JMenuItem(EventType.SURFACE_TRANSPORT.getName(), EventType.SURFACE_TRANSPORT.getIcon());
    addSurfaceTransport
        .setToolTipText("Transports a surface vehicle from surface node to surface node");
    ...
    JMenuItem addFlightProcess =
        new JMenuItem(EventType.FLIGHT_TRANSPORT.getName(), EventType.FLIGHT_TRANSPORT.getIcon());
    addFlightProcess.setToolTipText(
        "Transports a stack from node to node under pre-specified flight constraints");
    ...
    JMenuItem addExplorationProcess =
        new JMenuItem(EventType.EXPLORATION.getName(), EventType.EXPLORATION.getIcon());
  ...
  /**
   * Prefills the events based on the currently-selected event. If another event is selected, the
   * new event is assigned the same time, one higher priority, and same location. If another process
   * is selected, the new event is assigned the time after duration, priority 1, and same location.
   * If a transport is selected, the new event is assigned the time after duration, priority 1, and
   * destination location. The new event is also prefixed with up to 5 characters of the name of the
   * mission.
   * 
  ...
  private void prefill(I_Event event) {
    if (missionPanel.getEventsTable().getRowCount() > 0) {
      I_Event prevEvent;
      if (missionPanel.getEventsTable().getSelectedRowCount() > 0 && missionPanel.getEventsTable()
          .getSelectedRow() < missionPanel.getEventsTable().getRowCount()) {
        prevEvent =
            missionPanel.getEventsTable().getEvent(missionPanel.getEventsTable().getSelectedRow());
      } else {
        prevEvent =
            missionPanel.getEventsTable().getEvent(missionPanel.getEventsTable().getRowCount() - 1);
      }
