...
/**
 * The data source interface allows for the abstracted loading of libraries and elements from a
 * general data source. Nodes, edges, and resources are handled by loading and maintaining <em>in
 * memory</em> a library of objects for usage in the scenario. Elements are handled differently,
 * where only previews are loaded and maintained in memory to save on space -- when an element is
 * used in a scenario (during a Create Elements event), the data source is queried to build a new
 * element object.
 * 
 * @author Paul Grogan
 */
public interface I_DataSource {
  ...
  /**
   * Deletes an edge from the data source. Also deletes any associated objects (e.g. burns for space
   * edges).
   * 
  ...
  /**
   * Deletes an element from the data source. Also deletes all associated part applications and
   * states.
   * 
  ...
  /**
   * Deletes a node from the data source. If node is used in any edges in the edge library, this
  ...
  /**
   * Deletes a resource from the data source. If resource is used in any elements, demand models, or
   * demands, this method will fail (return false).
   * 
  ...
  /**
   * Gets the list of resources from the resource library, filtered to only include the continuous
   * resources.
   * 
  ...
  /**
   * Gets the list of resources from the resource library, filtered to only include the discrete
   * resources.
   * 
   ...
   */
  public void loadLibraries(boolean updateNodes, boolean updateEdges, boolean updateResources)
      throws Exception;
  ...
  /**
   * Save an edge to the data source and updates the edge library accordingly. Updates existing
   * entry if modified or inserts a new entry (assigning a new primary key) if created. Also saves
   * any associated objects (e.g. burns for a space edge).
   * 
   * @param edge the edge
   */
  ...
  /**
   * Saves an element to the data source and updates the element preview library accordingly.
   * Updates existing entry if modified or inserts a new entry (assigning a new primary key) if
   * created. Also saves all associated part applications and states.
   * 
  ...
  /**
   * Saves a node to the data source and updates the node library accordingly. Updates existing
   * entry if modified or inserts a new entry (assigning a new primary key) if created.
   * 
   * @param node the node
   */
  ...
  /**
   * Saves a resource to the data source and updates the resource library accordingly. Updates
   * existing entry if modified or inserts a new entry (assigning a new primary key) if created.
   * 
  ...
  /**
   * Runs a validation script on the data source, checking for any parse errors.
   * 
