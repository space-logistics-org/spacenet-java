...
public class RepairabilityTab extends JSplitPane {
  ...
  public RepairabilityTab(DemandsTab tab) {
    ...
    c.gridx++;
    c.weightx = 1;
    autoRepairButton = new JButton("Auto-Repair",
  ...
  /**
   * Automatically selects the best set of items to repair for a single mission.
   */
  private void autoRepair() {
    ...
    for (RepairItem item : repairItems) {
      if (repairTime >= item.getMeanRepairTime()) {
        ...
        logRepairItem(index, item.getDemand().getResource(), item.getDemand().getAmount(),
            item.getElement());
        ...
        repairTime = 0;
        logRepairItem(index, item.getDemand().getResource(), amount, item.getElement());
      } else if (repairTime >= item.getUnitMeanRepairTime()) {
        // otherwise can repair a whole portion of the demand
        int quantity = (int) Math.floor(
            Math.min(item.getDemand().getAmount(), repairTime / item.getUnitMeanRepairTime()));
        repairTime -= quantity * item.getUnitMeanRepairTime();
        logRepairItem(index, item.getDemand().getResource(), quantity, item.getElement());
      }
    }
  }
   ...
   */
  private void logRepairItem(int missionIndex, I_Resource resource, double amountRepaired,
      I_Element element) {
    Mission mission = getScenario().getMissionList().get(missionIndex);
    for (RepairItem item : getScenario().getRepairedItems().get(mission)) {
      if (item.getDemand().getResource().equals(resource) && item.getElement().equals(element)) {
    ...
    getScenario().getRepairedItems().get(mission)
        .add(new RepairItem(new Demand(resource, amountRepaired), element));
  }
