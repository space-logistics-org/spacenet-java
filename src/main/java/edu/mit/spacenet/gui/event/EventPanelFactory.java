...
public class EventPanelFactory {
  ...
  public static AbstractEventPanel createEventPanel(EventDialog eventDialog, I_Event event) {
    switch (event.getEventType()) {
      case CREATE:
        return new CreateEventPanel(eventDialog, (CreateEvent) event);
      case MOVE:
        return new MoveEventPanel(eventDialog, (MoveEvent) event);
      case REMOVE:
        return new RemoveEventPanel(eventDialog, (RemoveEvent) event);
      case RECONFIGURE:
        return new ReconfigureEventPanel(eventDialog, (ReconfigureEvent) event);
      case RECONFIGURE_GROUP:
        return new ReconfigureGroupEventPanel(eventDialog, (ReconfigureGroupEvent) event);
      case ADD:
        return new AddEventPanel(eventDialog, (AddEvent) event);
      case TRANSFER:
        return new TransferEventPanel(eventDialog, (TransferEvent) event);
      case DEMAND:
        return new DemandEventPanel(eventDialog, (DemandEvent) event);
      case EVA:
        return new EvaEventPanel(eventDialog, (EvaEvent) event);
      case EXPLORATION:
        return new ExplorationProcessPanel(eventDialog, (ExplorationProcess) event);
      case BURN:
        return new BurnEventPanel(eventDialog, (BurnEvent) event);
      case SPACE_TRANSPORT:
        return new SpaceTransportPanel(eventDialog, (SpaceTransport) event);
      case SURFACE_TRANSPORT:
        return new SurfaceTransportPanel(eventDialog, (SurfaceTransport) event);
      case FLIGHT_TRANSPORT:
        return new FlightTransportPanel(eventDialog, (FlightTransport) event);
      default:
