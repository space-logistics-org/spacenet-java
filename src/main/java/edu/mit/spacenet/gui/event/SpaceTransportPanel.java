...
public class SpaceTransportPanel extends AbstractEventPanel {
  ...
  private void initialize() {
    ...
    btnAddBurn.addActionListener(new ActionListener() {
      public void actionPerformed(ActionEvent e) {
        ((BurnPanel) tabbedBurnPane.getSelectedComponent()).sequenceModel
            .addElement(new BurnStageItem(BurnStageItem.BURN,
                ((CheckBoxNode) elementTree.getSelectionPath().getLastPathComponent())
                    .getElement()));
        updateView();
      }
    });
    btnAddStage.addActionListener(new ActionListener() {
      public void actionPerformed(ActionEvent e) {
        ((BurnPanel) tabbedBurnPane.getSelectedComponent()).sequenceModel
            .addElement(new BurnStageItem(BurnStageItem.STAGE,
                ((CheckBoxNode) elementTree.getSelectionPath().getLastPathComponent())
                    .getElement()));
        updateView();
      }
  ...
  public void updateView() {
    ...
    for (Edge e : SpaceNetFrame.getInstance().getScenarioPanel().getScenario().getNetwork()
        .getEdges()) {
      if (e.getOrigin().equals(getEventDialog().getNode()) && e instanceof SpaceEdge) {
    ...
    for (int burn = 0; burn < ((SpaceEdge) ddlTrajectory.getSelectedItem()).getBurns()
        .size(); burn++) {
  ...
  private void createBurnPanels() {
    ...
    for (int burn = 0; burn < trajectory.getBurns().size(); burn++) {
      ...
      panel.btnClear.addActionListener(new ActionListener() {
        public void actionPerformed(ActionEvent e) {
          for (BurnStageItem o : ((BurnPanel) tabbedBurnPane.getSelectedComponent()).sequenceList
      ...
      panel.sequenceList.addListSelectionListener(new ListSelectionListener() {
        public void valueChanged(ListSelectionEvent e) {
          if (!((BurnPanel) tabbedBurnPane.getSelectedComponent()).sequenceList
              .isSelectionEmpty()) {
  ...
  private void updateDeltaV() {
    try {
      ...
      for (Object o : elementModel.getTopCheckedElements()) {
        ...
        if (o instanceof PropulsiveVehicle && ((PropulsiveVehicle) o).getOmsIsp() > 0) {
          omsFuelMap.put((PropulsiveVehicle) o,
              ((PropulsiveVehicle) o).getOmsFuelTank().getAmount());
        }
        if (o instanceof PropulsiveVehicle && ((PropulsiveVehicle) o).getRcsIsp() > 0) {
          rcsFuelMap.put((PropulsiveVehicle) o,
              ((PropulsiveVehicle) o).getRcsFuelTank().getAmount());
        }
      }
      ...
      for (int burn = 0; burn < ((SpaceEdge) ddlTrajectory.getSelectedItem()).getBurns()
        ...
        for (Object o : burnPanel.sequenceModel.toArray()) {
          ...
          if (i.getBurnStage().equals(BurnStageItem.STAGE)) {
            ...
            if (b.getBurnType().equals(BurnType.OMS)) {
              if (Formulae.getRequiredFuelMass(stackMass, deltaVReq, v.getOmsIsp()) > omsFuelMap
                  .get(v)) {
                double deltaVAchieved =
                    Formulae.getAchievedDeltaV(stackMass, v.getOmsIsp(), omsFuelMap.get(v));
              ...
              if (Formulae.getRequiredFuelMass(stackMass, deltaVReq, v.getRcsIsp()) > rcsFuelMap
                  .get(v)) {
                double deltaVAchieved =
                    Formulae.getAchievedDeltaV(stackMass, v.getRcsIsp(), rcsFuelMap.get(v));
                // System.out.println("dv achieved: " + deltaVAchieved);
                deltaVReq -= deltaVAchieved;
        ...
        burnPanel.deltaV.setString(deltaVFormat.format((b.getDeltaV() - deltaVReq)) + " / "
            + b.getDeltaV() + " m/s (" + b.getBurnType().getName() + ")");
      }
    } catch (Exception ex) {
      ex.printStackTrace();
      JOptionPane.showMessageDialog(this, "Sorry, an unhandled exception occurred.",
          "Unhandled Exception in SpaceTransportGUI.updateDeltaV", JOptionPane.WARNING_MESSAGE);
    }
  }
    ...
    if (elementTree.getSelectionCount() > 0) {
      ...
      /* TODO: add check for previous stage */) {
        ...
        if (((CheckBoxNode) elementTree.getSelectionPath().getLastPathComponent())
          ...
          if ((((SpaceEdge) ddlTrajectory.getSelectedItem()).getBurns()
              .get(tabbedBurnPane.getSelectedIndex()).getBurnType().equals(BurnType.OMS)
              && v.getOmsFuelTank() != null)
              || (((SpaceEdge) ddlTrajectory.getSelectedItem()).getBurns()
                  .get(tabbedBurnPane.getSelectedIndex()).getBurnType().equals(BurnType.RCS)
