    ...
    if (returnVal == JFileChooser.APPROVE_OPTION) {
      SwingWorker<Void, Void> worker = new SwingWorker<Void, Void>() {
        public Void doInBackground() {
          ...
          try {
            spaceNetFrame.openScenario(
                spaceNetFrame.getScenarioChooser().getSelectedFile().getAbsolutePath());
          } catch (Exception ex) {
            ...
            JOptionPane.showMessageDialog(null,
                "An error of type \"" + ex.getClass().getSimpleName() + "\" occurred while opening "
