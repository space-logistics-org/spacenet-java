...
public class DataSourceDialog extends JDialog {
  ...
  private void buildDialog() {
    ...
    dataSourceTypeCombo
        .setSelectedItem(dataSource == null ? DataSourceType.NONE : dataSource.getDataSourceType());
    ...
    libraryTabs.addTab("Nodes",
        new ImageIcon(getClass().getClassLoader().getResource("icons/bullet_black.png")),
        nodeScroll);
    ...
    libraryTabs.addTab("Edges",
        new ImageIcon(getClass().getClassLoader().getResource("icons/edge_red.png")), edgeScroll);
    ...
    libraryTabs.addTab("Resources",
        new ImageIcon(getClass().getClassLoader().getResource("icons/bullet_blue.png")),
        resourceScroll);
      ...
      public void mouseClicked(MouseEvent e) {
        if (e.getClickCount() == 2 && elementTable.getSelectedRowCount() == 1) {
          ElementPreview elementpreview =
              dataSource.getElementPreviewLibrary().get(elementTable.getSelectedRow());
          ...
          try {
            ...
            JOptionPane.showMessageDialog(getThis(),
                ...
                "Element Load Error", JOptionPane.ERROR_MESSAGE);
          }
        }
      }
    });
    ...
    libraryTabs.addTab("Elements",
        new ImageIcon(getClass().getClassLoader().getResource("icons/lunar_lander.png")),
        elementScroll);
    ...
    addButton = new JButton("Add New",
        new ImageIcon(getClass().getClassLoader().getResource("icons/add.png")));
      ...
      public void actionPerformed(ActionEvent e) {
        switch (libraryTabs.getSelectedIndex()) {
          ...
          case 3:
            ...
            // TODO turn sizing model back on SizingModelDialogEditor.createAndShowGUI(getThis(),
            // ElementType.ELEMENT);
            break;
        }
      }
    ...
    deleteButton.addActionListener(new ActionListener() {
      public void actionPerformed(ActionEvent e) {
        int response = JOptionPane.showConfirmDialog(getThis(),
            "Are you sure you want to permanently delete objects?", "SpaceNet Warning",
        ...
        if (response == JOptionPane.YES_OPTION) {
          ...
          switch (libraryTabs.getSelectedIndex()) {
              ...
              for (int i : reverse(nodeTable.getSelectedRows())) {
                try {
                  boolean isSuccessful =
                      dataSource.deleteNode(dataSource.getNodeLibrary().get(i).getTid());
                  if (!isSuccessful) {
            ...
            case 1:
              for (int i : reverse(edgeTable.getSelectedRows())) {
                try {
                  boolean isSuccessful =
                      dataSource.deleteEdge(dataSource.getEdgeLibrary().get(i).getTid());
                  if (!isSuccessful) {
            ...
            case 2:
              for (int i : reverse(resourceTable.getSelectedRows())) {
                try {
                  boolean isSuccessful =
                      dataSource.deleteResource(dataSource.getResourceLibrary().get(i).getTid());
                  if (!isSuccessful) {
                    errorMessages
                        .add("Resource " + dataSource.getResourceLibrary().get(i).getTid());
                  }
                } catch (Exception e1) {
                  e1.printStackTrace();
                }
            ...
            case 3:
              for (int i : reverse(elementTable.getSelectedRows())) {
                try {
                  boolean isSuccessful =
                      dataSource.deleteElement(dataSource.getElementPreviewLibrary().get(i).ID);
                  if (!isSuccessful) {
          ...
          if (errorMessages.size() > 0) {
            String tempError =
                "The following objects could not be deleted because of existing references: \n";
            for (int j = 0; j < errorMessages.size(); j++) {
              tempError = tempError.concat(" - " + errorMessages.get(j) + "\n");
            }
            JOptionPane.showMessageDialog(getThis(), tempError, "SpaceNet Error",
    ...
    JButton cancelButton = new JButton("Cancel",
        new ImageIcon(getClass().getClassLoader().getResource("icons/database.png")));
  ...
  public void paint(Graphics g) {
    ...
    editButton.setEnabled(dataSourcePanel.canLoad() && (nodeTable.getSelectedRowCount() == 1
        || edgeTable.getSelectedRowCount() == 1 || resourceTable.getSelectedRowCount() == 1
        || elementTable.getSelectedRowCount() == 1));
    deleteButton.setEnabled(dataSourcePanel.canLoad()
        && (nodeTable.getSelectedRowCount() > 0 || edgeTable.getSelectedRowCount() > 0
            || resourceTable.getSelectedRowCount() > 0 || elementTable.getSelectedRowCount() > 0));
  }

  /**
   * Requests that the dialog update its fields based on the model and also requests the data source
   * panel to update itself.
   */
