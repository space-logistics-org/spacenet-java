...
public class PropulsiveVehiclePanel extends AbstractElementPanel {
  ...
  private SpinnerNumberModel omsIspModel, maxOmsFuelModel, omsFuelModel, rcsIspModel,
      maxRcsFuelModel, rcsFuelModel;
  private JSpinner omsIspSpinner, maxOmsFuelSpinner, omsFuelSpinner, rcsIspSpinner,
      maxRcsFuelSpinner, rcsFuelSpinner;
  ...
  private void buildPanel() {
    ...
    omsResourceCombo.addItemListener(new ItemListener() {
      public void itemStateChanged(ItemEvent e) {
        if (e.getStateChange() == ItemEvent.SELECTED && omsResourceCombo.getSelectedItem() != null
    ...
    sharedCheck
        .setToolTipText("Share fuel between the orbital maneuvering and reaction control systems");
    ...
    rcsResourceCombo.addItemListener(new ItemListener() {
      public void itemStateChanged(ItemEvent e) {
        if (e.getStateChange() == ItemEvent.SELECTED && rcsResourceCombo.getSelectedItem() != null
  ...
  private void initialize() {
    ...
    for (I_Resource r : SpaceNetFrame.getInstance().getScenarioPanel().getScenario().getDataSource()
        .getResourceLibrary()) {
    ...
    for (ClassOfSupply cos : ClassOfSupply.values()) {
      if (cos.equals(ClassOfSupply.COS1) || cos.isSubclassOf(ClassOfSupply.COS1)) {
        omsResourceCombo.addItem(new GenericResource(cos));
        rcsResourceCombo.addItem(new GenericResource(cos));
      }
    }
    if (vehicle.getOmsIsp() > 0)
      omsCheck.setSelected(true);
    else
      omsCheck.setSelected(false);

    if (vehicle.getRcsIsp() > 0)
      rcsCheck.setSelected(true);
    else
      rcsCheck.setSelected(false);
  ...
  public void saveElement() {
    if (omsCheck.isSelected()) {
      ...
      if (vehicle.getOmsFuelTank() != null)
        omsFuelTank = vehicle.getOmsFuelTank();
      else {
    ...
    if (rcsCheck.isSelected() && !sharedCheck.isSelected()) {
      ...
      if (vehicle.getRcsFuelTank() != null)
        rcsFuelTank = vehicle.getRcsFuelTank();
      else {
