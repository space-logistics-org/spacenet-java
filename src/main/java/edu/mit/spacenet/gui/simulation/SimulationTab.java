...
/**
 * Tab component for running the full simulation, analyzing errors, and visualizing the outputs.
 * 
 * @author Paul Grogan
 */
public class SimulationTab extends JSplitPane {
  ...
  public SimulationTab(ScenarioPanel scenarioPanel) {
    ...
    detailedExplorationsCheck.setToolTipText(
        "Detailed explorations are not necessary if using mission-level demand models to capture EVA demands.");
    ...
    detailedEvasCheck.setToolTipText(
        "Detailed EVAs are not necessary if using mission-level demand models to capture EVA demands.");
    ...
    simulateButton = new JButton("Simulate",
        new ImageIcon(getClass().getClassLoader().getResource("icons/lightning_go.png")));
            ...
            try {
              ...
              if (simulator.getErrors().size() > 0) {
                JOptionPane.showMessageDialog(tabs,
                    "The simulation completed with " + simulator.getErrors().size() + " error"
                        + (simulator.getErrors().size() == 1 ? "" : "s") + ".",
                    "SpaceNet Simulation Errors", JOptionPane.WARNING_MESSAGE);
              }
            } catch (Exception ex) {
              ex.printStackTrace();
            }
