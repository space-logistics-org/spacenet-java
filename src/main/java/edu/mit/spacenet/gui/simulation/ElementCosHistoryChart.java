...
/**
 * Chart for displaying the history of elements' contents broken down by class of supply.
 * 
 * @author Paul Grogan
 */
public class ElementCosHistoryChart extends JSplitPane {
  ...
  public void updateView() {
    ...
    if (selectedElement != null)
      elementCombo.setSelectedItem(selectedElement);
    ...
    if (elementCombo.getSelectedItem() != null) {
      for (ClassOfSupply cos : cosModel.getSelectedObjects()) {
        ...
        for (SimNetwork network : tab.getSimulator().getNetworkHistory()) {
          I_Element element = network.getNetwork().getRegistrar()
              .get(((I_Element) elementCombo.getSelectedItem()).getUid());
          if (element == null)
            element = network.getNetwork().getRemovedRegistrar()
                .get(((I_Element) elementCombo.getSelectedItem()).getUid());
          if (element != null) {
            Hour hour =
                new Hour(DateFunctions.getDate(getScenario().getStartDate(), network.getTime()));
            if (!firstPoint && !linearizeDataCheck.isSelected())
