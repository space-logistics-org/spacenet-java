  ...
  public TransferDemandsTable() {
    ...
    getColumnModel().getColumn(1).setCellRenderer(new DefaultTableCellRenderer() {
      ...
      public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected,
        ...
        setText(format.format(value) + " " + ((TransferDemandsModel) getModel())
            .getContainerContents(row).getResource().getUnits());
        return this;
      }
    });
  ...
  private class TransferDemandsModel extends AbstractTableModel {
    ...
    public int getRowCount() {
      if (container == null)
        ...
        return container.getContents().keySet().size();
    }

    public Object getValueAt(int row, int col) {
      if (col == 0)
        return getContainerContents(row).getResource();
      else if (col == 1)
        return getContainerContents(row).getAmount();
      else
        return getTransferDemand(row) == null ? 0 : getTransferDemand(row).getAmount();
    }
    ...
    public Demand getContainerContents(int row) {
      ...
      for (I_Resource resource : container.getContents().keySet()) {
        lastRowSeen++;
        if (lastRowSeen == row)
          return new Demand(resource, container.getContents().get(resource));
      }
      return null;
    }
