...
public enum ScenarioType {
  ...
  /**
   * Method to get a particular instance of a scenario type based on a case-insensitive string.
   * 
  ...
  public static ScenarioType getInstance(String name) {
    if (name.equals(ISS.getName()))
      return ISS;
    else if (name.equals(LUNAR.getName()))
      return LUNAR;
    else if (name.equals(MOON_ONLY.getName()))
      return MOON_ONLY;
    else if (name.equals(MARTIAN.getName()))
      return MARTIAN;
    else if (name.equals(MARS_ONLY.getName()))
      return MARS_ONLY;
    else
      return null;
  }
  ...
  public boolean isNodeVisible(Node node) {
    switch (this) {
      case ISS:
        if (node.getBody() == Body.EARTH)
          return true;
        else
          return false;
      case LUNAR:
        if (node.getBody() == Body.EARTH || node.getBody() == Body.MOON)
          return true;
        else
          return false;
      case MOON_ONLY:
        if (node.getBody() == Body.MOON)
          return true;
        else
          return false;
      case MARTIAN:
        if (node.getBody() == Body.EARTH || node.getBody() == Body.MARS)
          return true;
        else
          return false;
      case MARS_ONLY:
        if (node.getBody() == Body.MARS)
          return true;
        else
          return false;
      case SOLAR_SYSTEM:
  ...
  public boolean isEdgeVisible(Edge edge) {
    if (isNodeVisible(edge.getOrigin()) && isNodeVisible(edge.getDestination()))
      return true;
    else
      return false;
  }
