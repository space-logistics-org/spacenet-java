...
public class EdgeEditorDialog extends JDialog {
  ...
  private void buildDialog() {
    ...
    /*
     * c.gridy++; c.gridx = 0; c.weightx = 0; c.anchor = GridBagConstraints.LINE_START; c.fill =
     * GridBagConstraints.NONE; JButton addNodeButton = new JButton("Add Nodes", new
     * ImageIcon(getClass().getClassLoader().getResource("icons/asterisk_add.png")));
     * addNodeButton.addActionListener(new ActionListener() { public void
     * actionPerformed(ActionEvent e) { NodeEditorDialog.createAndShowGUI(dialog, new
     * OrbitalNode()); } }); contentPanel.add(addNodeButton,c);
     */
    ...
    okButton = new JButton(edge.getTid() < 0 ? "Add" : "Save",
  ...
  public void saveEdge() {
    if (edgePanel.isEdgeValid()) {
      ...
      try {
        ...
        e.printStackTrace();
        JOptionPane.showMessageDialog(dialog, "SpaceNet Errror",
            "An error occurred while saving the edge.", JOptionPane.ERROR_MESSAGE);
      }
    }
  }
