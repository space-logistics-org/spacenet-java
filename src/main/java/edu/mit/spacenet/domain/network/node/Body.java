...
public enum Body implements Serializable {
  ...
  /** The moon */
  MOON("Moon", Color.LIGHT_GRAY, "/icons/moon.png", EARTH, 0.0123, 0.273, 0.00257,
      "icons/moon_icon.png"),
  ...
  /** Phobos */
  PHOBOS("Phobos", Color.DARK_GRAY, null, MARS, 1.8 * Math.pow(10, -9), 11.1 / 6378.,
      9377.2 / (150 * Math.pow(10, 6)), "icons/phobos_icon.png"),

  /** Deimos */
  DEIMOS("Deimos", Color.DARK_GRAY, null, MARS, 0.25 * Math.pow(10, -9), 6.2 / 6378.,
      23460 / (150 * Math.pow(10, 6)), "icons/deimos_icon.png"),
  ...
  /**
   * Method to get a particular instance of a body based on a case-insensitive string.
   * 
  ...
  public static Body getInstance(String name) {
    for (Body body : Body.values()) {
      if (body.getName().toLowerCase().equals(name.toLowerCase()))
        return body;
    }
    return null;
  }
   ...
   */
  private Body(String name, Color color, String imageUrl, Body parent, double mass,
      double planetaryRadius, double orbitalRadius, String iconUrl) {
    this.name = name;
    this.color = color;
    if (imageUrl != null) {
      try {
